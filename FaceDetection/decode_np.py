...
class Decode(object):
    ...
    # 处理一张图片
    def detect_image(self, image, draw_image, img_name):
        ...
        if boxes is not None and draw_image:
            self.draw(image, boxes, scores, classes, img_name)
    ...
    # Draw Classes
    def draw(self, image, boxes, scores, classes, img_name):
        ...
        for box, score, cl in zip(boxes, scores, classes):
            ...
            if cl == 0:     # only for person (class # 0)
                crop_img = image[top:bottom, left:right]
                new_name = img_name.split('.')[0] + '_' + str(idx) + '.png'
                cv2.imwrite('data/person_yolo/' + new_name, crop_img)
                f = open('txt/person/' + img_name.split('.')[0] + '.txt', "a")
                f.write(new_name + ',' + str(left) + ',' + str(top) + ',' + str(right) + ',' + str(bottom) + '\n')
                f.close()
